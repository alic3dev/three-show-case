...
const NUM_KEYPOINTS: number = 468
const NUM_IRIS_KEYPOINTS: number = 5

const GREEN: string = '#32EEDB'
const RED: string = '#FF2C35'
...
export const FaceDetectionApp: AppComponent = (): React.ReactElement => {
  ...
  const settingsRef = React.useRef<{
    ...
    contours: boolean
    twoDMesh: boolean
    twoDTriangles: boolean
    twoDBoundingBox: boolean
    irisOutline: boolean
  }>({
    threeDMesh: true,
    threeDWire: false,
    ...
    twoDMesh: false,
    twoDTriangles: true,
    twoDBoundingBox: false,
    irisOutline: false,
  })

  const faceDetectorRef = React.useRef<{
    ctx: CanvasRenderingContext2D | null
    detector: faceLandmarksDetection.FaceLandmarksDetector | null
  }>()

  React.useEffect((): (() => void) | void => {
    let updateVideoAnimationFrameHandle: number
    let aborted: boolean = false

    const initializeCamera = async (): Promise<void> => {
      if (!videoRef.current || !canvasRef.current || aborted) {
        return
      }
      ...
      if (!faceDetectorRef.current) {
        ...
        // FIXME: Adjust this and normalize
        const size = { width: 1920, height: 1080 }
        // const size = { width: 640, height: 480 }
        // const mobileSize = { width: 360, height: 270 }
        ...
        await new Promise<void>(
          (resolve: () => void, reject: () => void): void => {
            ...
            videoRef.current.onloadedmetadata = (): void => resolve()
          },
        )
        ...
        const detector: faceLandmarksDetection.FaceLandmarksDetector | null =
      ...
      const updateVideo = async (): Promise<void> => {
        if (!faceDetectorRef.current || aborted) return

        const ctx: CanvasRenderingContext2D | null = faceDetectorRef.current.ctx
        const detector: faceLandmarksDetection.FaceLandmarksDetector | null =
          faceDetectorRef.current.detector

        if (!ctx || !videoRef.current || !detector) return

        ctx.clearRect(0, 0, ctx.canvas.width, ctx.canvas.height)

        if (settingsRef.current.feed) {
          ctx.drawImage(
            videoRef.current,
            0,
            0,
        ...
        try {
          faces = await detector.estimateFaces(videoRef.current, {
            flipHorizontal: false,
          })
        } catch (error: unknown) {
          detector.dispose()
          faceDetectorRef.current.detector = null
        ...
        const face: faceLandmarksDetection.Face | undefined = faces[0]

        if (face) {
          const keypoints: [number, number][] = face.keypoints.map(
            (keypoint: faceLandmarksDetection.Keypoint): [number, number] => [
              keypoint.x,
              keypoint.y,
            ],
          )

          if (settingsRef.current.twoDBoundingBox) {
            ctx.strokeStyle = RED
            ctx.lineWidth = 1
          ...
          if (settingsRef.current.twoDMesh) {
            if (settingsRef.current.twoDTriangles) {
              ctx.strokeStyle = GREEN
              ctx.lineWidth = 0.5

              for (let i: number = 0; i < TRIANGULATION.length / 3; i++) {
                const points: [number, number][] = [
                  TRIANGULATION[i * 3],
                  TRIANGULATION[i * 3 + 1],
                  TRIANGULATION[i * 3 + 2],
                ].map((index: number): [number, number] => keypoints[index])
              ...
              for (let i: number = 0; i < NUM_KEYPOINTS; i++) {
                ctx.beginPath()
                ctx.arc(keypoints[i][0], keypoints[i][1], 1, 0, 2 * Math.PI)
                ctx.fill()
              }
            }
          }
          ...
          if (
            settingsRef.current.irisOutline &&
            keypoints.length > NUM_KEYPOINTS + NUM_IRIS_KEYPOINTS
          ) {
            ctx.strokeStyle = RED
            ctx.lineWidth = 1

            const leftCenter: [number, number] = keypoints[NUM_KEYPOINTS]

            const leftDiameterY: number = distance(
              keypoints[NUM_KEYPOINTS + 4],
              keypoints[NUM_KEYPOINTS + 2],
            )

            const leftDiameterX: number = distance(
            ...
            const rightDiameterY: number = distance(
              keypoints[NUM_KEYPOINTS + NUM_IRIS_KEYPOINTS + 2],
              keypoints[NUM_KEYPOINTS + NUM_IRIS_KEYPOINTS + 4],
            )

            const rightDiameterX: number = distance(
          ...
          if (settingsRef.current.contours) {
            const contours: {
              [label: string]: number[]
            ...
            for (const [label, contour] of Object.entries(contours)) {
              ...
              const path: [number, number][] = contour.map(
                (index: number): [number, number] => keypoints[index],
              )

              if (
                path.every(
                  (value: [number, number]): boolean => value != undefined,
                )
              ) {
                const pathLengthHalf: number = Math.floor(path.length / 2)
                const pathLengthQuarter: number = Math.floor(path.length / 4)
                ...
                switch (label) {
                  case 'lips':
                    drawPath(
                      ctx,
                      [
                        ...path.slice(0, pathLengthQuarter),
                        ...path
                          .slice(pathLengthQuarter, pathLengthHalf)
                          .reverse(),
                      ],
                      true,
                    )
                    ...
                    drawPath(
                      ...
                      [
                        ...path.slice(
                          pathLengthHalf,
                          pathLengthHalf + pathLengthQuarter,
                        ),
                        ...path
                          .slice(pathLengthHalf + pathLengthQuarter)
                          .reverse(),
                      ],
                      true,
                    )
                  ...
                  case 'rightEye':
                    drawPath(
                      ctx,
                      [
                        ...path.slice(0, pathLengthHalf),
                        ...path.slice(pathLengthHalf).reverse(),
                      ],
                      true,
                    )
                    break
              ...
              points.push(
                ...[
                  ...
                  (index: number): THREE.Vector3 =>
                    new THREE.Vector3(
                      face.keypoints[index].x,
                      -face.keypoints[index].y,
                      face.keypoints[index].z ?? 0,
                    ),
                ),
              )
            }

            rendererProperties.current.faceMesh.geometry.setFromPoints(points)
            rendererProperties.current.faceMesh.visible =
              settingsRef.current.threeDMesh

            if (!rendererProperties.current.faceWireLineSegments) {
              const faceWireGeometry: THREE.EdgesGeometry =
                new THREE.EdgesGeometry(
                  rendererProperties.current.faceMesh.geometry,
                )

              const faceWireMaterial: THREE.LineBasicMaterial =
                new THREE.LineBasicMaterial({ color: GREEN })

              const faceWireLineSegments: THREE.LineSegments =
                new THREE.LineSegments(faceWireGeometry, faceWireMaterial)

              faceWireLineSegments.scale.set(
                rendererProperties.current.faceMesh.scale.x,
                rendererProperties.current.faceMesh.scale.y,
                rendererProperties.current.faceMesh.scale.z,
              )

              faceWireLineSegments.position.set(
                rendererProperties.current.faceMesh.position.x,
                rendererProperties.current.faceMesh.position.y,
                rendererProperties.current.faceMesh.position.z,
              )

              faceWireLineSegments.visible = settingsRef.current.threeDWire
              faceWireLineSegments.frustumCulled = false

              rendererProperties.current.faceWireLineSegments =
                faceWireLineSegments

              rendererProperties.current.scene.add(faceWireLineSegments)
            } else {
              rendererProperties.current.faceWireLineSegments.geometry.dispose()
              rendererProperties.current.faceWireLineSegments.geometry =
                new THREE.EdgesGeometry(
                  rendererProperties.current.faceMesh.geometry,
                )

              rendererProperties.current.faceWireLineSegments.visible =
                settingsRef.current.threeDWire
            }
          }
        } else {
          if (rendererProperties.current) {
            rendererProperties.current.faceMesh.visible = false
          }
          if (rendererProperties.current?.faceWireLineSegments) {
            rendererProperties.current.faceWireLineSegments.visible = false
          }
        }

        updateVideoAnimationFrameHandle = requestAnimationFrame(updateVideo)
      }
      updateVideoAnimationFrameHandle = requestAnimationFrame(updateVideo)
    }
    ...
    return (): void => {
      aborted = true
      window.cancelAnimationFrame(updateVideoAnimationFrameHandle)
    }
  }, [])
  ...
  React.useEffect((): (() => void) | void => {
    ...
    if (!rendererProperties.current) {
      ...
      const faceGeometry: THREE.BufferGeometry = new THREE.BufferGeometry()
      const faceMaterial: THREE.MeshPhongMaterial = new THREE.MeshPhongMaterial(
        {
          color: 0xffffff,
      ...
      const faceMesh: THREE.Mesh = new THREE.Mesh(faceGeometry, faceMaterial)
      faceMesh.castShadow = true
      faceMesh.receiveShadow = true

      const faceScale: number = 0.002
      faceMesh.scale.set(faceScale, faceScale, faceScale)

      faceMesh.position.set(-1.9, 1.07, 2.129)
      faceMesh.visible = settingsRef.current.threeDMesh
      ...
      scene.traverse((object: THREE.Object3D<THREE.Object3DEventMap>): void => {
    ...
    const animate: XRFrameRequestCallback = (): void => {
      ...
      if (heldKeys['a']) {
        rendererProperties.current?.faceMesh.translateX(0.01)
      }
      ...
      if (heldKeys['q']) {
        rendererProperties.current?.faceMesh.translateY(0.01)
      }
      ...
      if (heldKeys['space']) {
        console.log(rendererProperties.current?.faceMesh.position)
      }
    ...
    const threeDFolder: GUI = panel.addFolder('3D')
    threeDFolder.add(settingsRef.current, 'threeDMesh').onChange((): void => {
      if (rendererProperties.current) {
        rendererProperties.current.faceMesh.visible =
          settingsRef.current.threeDMesh
      }
    })
    threeDFolder.add(settingsRef.current, 'threeDWire').onChange((): void => {
      if (rendererProperties.current?.faceWireLineSegments) {
        rendererProperties.current.faceWireLineSegments.visible =
          settingsRef.current.threeDWire
      }
    ...
    const twoDFolder: GUI = panel.addFolder('2D')
    twoDFolder.add(settingsRef.current, 'feed')
    twoDFolder.add(settingsRef.current, 'twoDBoundingBox')
    twoDFolder.add(settingsRef.current, 'twoDMesh')
    twoDFolder.add(settingsRef.current, 'twoDTriangles')
    twoDFolder.add(settingsRef.current, 'contours')
    twoDFolder.add(settingsRef.current, 'irisOutline')
